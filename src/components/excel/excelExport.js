...
export const ExcelExportChannels = ({data}) => {
    //console.log("valuesToFilter no excel", usersDealerInfo)
    ...
    const createDownloadData = () => {
        ...
        handleExport().then((url) => {
            ...
            const downloadAnchorNode = document.createElement('a');
            downloadAnchorNode.setAttribute('href', url);
            downloadAnchorNode.setAttribute('download', 'RELATORIO DE CANAIS '+'.xlsx');
            downloadAnchorNode.click()
            downloadAnchorNode.remove()
        });
    }
    ...
    */}
        const title2 =[{A: 'Relatório - Canais'}]; 
        ...
        data.forEach(( row, idx) => {
            ...
            table2.push({
                A: row.channels_id,
                B: row.channels_name,
                C: row.channels_active === 1 ? "Ativo" : "Inativo",
    ...
    */}
        ...
        XLSX.utils.book_append_sheet(wb, sheet2, 'Canais');
        ...
        const dataInfo2 = {
            titleCell: 'A1',
            titleRange: 'A1:C1',
            tbodyRange: `A3:C${finalData2.length}`,
            theadRangeC: `G2:G${finalData2.length}`,
            theadRange: 
                headerIndexes2?.length >= 1
                ? `A${headerIndexes2[0] + 1}:C${headerIndexes2[0] + 1}`
...
export const ExcelExportVods = ({data}) => {
    //console.log("valuesToFilter no excel", usersDealerInfo)
    ...
    const createDownloadData = () => {
        ...
        handleExport().then((url) => {
            ...
            const downloadAnchorNode = document.createElement('a');
            downloadAnchorNode.setAttribute('href', url);
            downloadAnchorNode.setAttribute('download', 'RELATORIO DE VODS '+'.xlsx');
            downloadAnchorNode.click()
            downloadAnchorNode.remove()
        });
    }
    ...
    */}
        ...
        XLSX.utils.book_append_sheet(wb, sheet2, 'Vods');
